...
public class OtobokeScript : MonoBehaviour
{
    enum State
    {
        Scatter,
    ...
    void FindNextStep()
    {
        if (transform.position == targetPosition)
        {
            ...
            switch (state)
            {
                ...
                case State.Chase:
                    Vector3 Target = Vector3.Distance(transform.position, player.position) < 8 ? scatterTarget : player.position;
                    Debug.DrawLine(Target + Vector3.right + (Vector3.up * 4), Target + Vector3.left + (Vector3.up * 4));
                    Debug.DrawLine(Target + Vector3.forward + (Vector3.up * 4), Target + Vector3.back + (Vector3.up * 4));
